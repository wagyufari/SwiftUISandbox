...
#if os(iOS)
    struct MultiTextFieldRep: UIViewRepresentable {
        @Binding var text: String
        var isFocused: Binding<Bool>?
        @Binding var height: CGFloat
        var font:UIFont
        var returnKeyType: MultiTextField.ReturnKeyType
        var onCommit: (() -> Void)?
        ...
        class Coordinator: NSObject, UITextViewDelegate {
            let rep: MultiTextFieldRep

            internal init(rep: MultiTextFieldRep) {
                self.rep = rep
            }
